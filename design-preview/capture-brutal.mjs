...
const SEL = '.status-bar.active .status-label';
const PERIOD = 0.9, N = 12;
...
for (const theme of ['brutal-dark','brutal-light']) {
  await page.goto(URL,{waitUntil:'networkidle'});
  ...
  console.log('wrote', out, N+'fr', Math.round(fs.statSync(out).size/1024)+'KB');
}
await browser.close();
console.log('done');
